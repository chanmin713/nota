...
## Loading Skeletons
- 적용 범위: 에디터 문서 본문, 사이드바(Outline/AI/Backlinks), 목록/검색 결과 패널
- 패턴: 블록 스켈레톤(heading/body/list/table rows), 사이드바 패널 스켈레톤, 버튼/툴바 플레이스홀더
- 지표: 최초 표시 ≤ 150ms, 완전 렌더 전 프레임 드랍 없이 전환, 스켈레톤 노출 최대 2s
- a11y: 스켈레톤 영역은 스크린리더에서 “로딩 중”으로 요약(aria-busy), 깜빡임 최소화
